<?php
/*
Template Name: Page Template Network Symbol
*/

get_header();
/*commercial product  work start*/
global $wp;

/*commercial product  work end;*/
 ?>
        ...
        <?php endwhile; ?>
            ...
            $args = array(
               ...
               if ($the_query->found_posts!=0)
				...
				/*commercial product work start*/
				$commercial 		= get_field("commercial_product",$post->ID );
                ...
                <div class="col-lg-4 col-md-5 col-sm-4 col-xs-12">
					...
					<div class="prodPriceSec">
						 <span class="addToCart addToCartTxt <?php if($commercial){echo "addToCartTxt";}?>">
							<?php
								if($userLogin && $download){
									...
									?>
									<a href="<?= get_the_permalink(240);?>?str=<?=$rand ?>">Download</a>
									<?php
								}
								else{
									if(!empty($prodId) && $prodId[$post->ID]==$post->ID){
									?>
									<a href="<?php echo wc_get_checkout_url() ; ?>">Download (Personal or Internal Use)</a>
									<?php
									}
									else{
					...
					if($commercial){
					 /*commercial product work start*/
					?>
					 <div class="prodPriceSec prodPriceSecCom">
						<span class="addToCart">
						<?php


								$commercial_product = get_product( $commercial->ID);
								if(!empty($prodId) && $prodId[$commercial->ID]==$commercial->ID){
									?>
									<a href="<?php echo wc_get_checkout_url() ; ?>">Download (Commercial Use)</a>
								<?php
								}
								else{
									?>
									<a class="button product_type_simple add_to_cart_button ajax_add_to_cart" href="<?php echo  home_url($wp->request)."/?add-to-cart=".$commercial->ID ; ?>">Download (Commercial Use)</a>
									<?php
								}
								?>
								<span class="prodPrice"><?php echo $commercial_product->get_price_html(); ?></span>
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
						<div class="innerBoxProd">
							...
							<div class="prodPriceSec">
								...
								<?php
									...
									if($userLogin && $download){
										 ...
										 $encryptedArr[$rand]['file_url'] 		=  $donwloadLink;
										?>
										<a href="<?= get_the_permalink(240);?>?str=<?=$rand ?>">Download (Personal or Internal Use)</a>
										<?php
									}
										...
										if($product->price==""){
										   ...
										   ?>
										   <a href="<?php echo   $freedownloadLink;?>" download>Download (Personal or Internal Use)</a>
										   <?php
										}
										else{
							...
							<div class="prodPriceSec prodPriceSecCom">
								<span class="prodPrice">
								<?php
									echo $commercial_product->get_price_html();
								 ?>
								</span>
								<span class="addToCart">
								<?php
									...
									if($userLogin && $download){
										 foreach($commercial_product->downloads as $k=>$v){
											$donwloadLink =  $commercial_product->downloads[$k]['file'];
										 }
										 $rand 									= 	md5(rand(10,100));
										 $encryptedArr[$rand]['file_url'] 		=  $donwloadLink;
										?>
										<a href="<?= get_the_permalink(240);?>?str=<?=$rand ?>">Download (Commercial Use)</a>
										<?php
									}
...
jQuery(document).ready(function($){
   ...
   $(".addToCartTxt a").html("Download (Personal or Internal Use)");
